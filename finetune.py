...
linear_params = {'kernel': w_initializer(rand.PRNGKey(42), (768, 768), np.float32),
                 'bias': b_initializer(rand.PRNGKey(42), (768,), np.float32)}
...
def stage_1_batch_update(params, other_params, src, dst, mask_enc, mask_dec, mask_dec_enc, labels, optimizer,
                         opt_state):
...
replicated_opt = jax.tree_map(lambda x: np.array([x] * n_devices), optimizer)
replicated_opt_state = jax.tree_map(lambda x: np.array([x] * n_devices), opt_state)
...
for _ in tqdm_epoch:
    ...
    for i in tqdm_batch:
        ...
        replicated_params, loss, replicated_opt_state = stage_1_batch_update(replicated_params, replicated_other_params,
                                                                             src, dst, mask_enc, mask_dec, mask_dec_enc,
                                                                             labels, replicated_opt,
