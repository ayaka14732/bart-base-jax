...
def load_sentences(split=Literal['train', 'dev', 'test']) -> list[tuple[str, str]]:
    ...
    if split == 'train':
        with fileinput.input(files=('{split}.en.txt', '../abc-cantonese-parallel-corpus/en.txt'), encoding='utf-8') as f:
            en = [line.rstrip('\n') for line in f]

        with fileinput.input(files=('{split}.yue.txt', '../abc-cantonese-parallel-corpus/yue.txt'), encoding='utf-8') as f:
            yue = [line.rstrip('\n') for line in f]
